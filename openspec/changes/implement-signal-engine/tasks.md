...
- [x] 11.1 启动恢复：outbox 未终态、结果追踪计划、今日 WS 计数、订阅重建、插件加载
- [x] 11.2 容量熔断（70/80/90）与数据保留清理任务（文档第 9.4/9.5 节）
- [x] 11.3 单元测试覆盖文档第 13.1 节全部条目
- [x] 11.4 集成测试覆盖文档第 13.2 节核心条目（供应商契约 fixture、WS 断线注入、Telegram 回放、SQLite 恢复）
- [x] 11.5 第一阶段验收：文档第 13.3 节 20 条核销 + `signal-core`/`plugin-sdk` spec 场景核销 + 与用户共同确认
