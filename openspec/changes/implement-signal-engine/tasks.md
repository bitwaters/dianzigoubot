...
- [ ] 2.1 迁移框架与 `001_init.sql`（文档第 9.3 节全部核心表、索引、唯一约束；`wallet_*` 聪明钱表 v1 建表不写入，v2 启用）
- [ ] 2.2 `storage_task` 优先级队列写入（策略事务 > 信号与安全快照 > 候选行情）、队列水位暂停规则、1m 行情内存合并
- [ ] 2.3 Repository 层基础 CRUD 与备份（SQLite Backup API）
...
- [ ] 5.1 G1/G3 ActionCable client（订阅/确认契约、双链独立连接）
- [ ] 5.2 断线退避/抖动/重连与鉴权失败停止；重连后订阅重建
- [ ] 5.3 G3 1 秒 K 线聚合（final 判定、W5/W10 窗口、缺口拒绝）与 G1 顶部池映射
- [ ] 5.4 订阅准入：100 物理上限 + WS 每日预算（`daily_credit_budget`）+ 候选淘汰
...
- [ ] 7.1 事件分发：内存队列 fan-out、分发超时（默认 5 秒）、熔断滑动窗口计数与告警、重启补发（至少一次投递）
- [ ] 7.2 插件加载：`plugins/<name>/register.py` 约定、`plugins.yaml` 启用顺序、加载失败隔离、同步（阻塞式）handler 注册拒绝
- [ ] 7.3 通知 API（outbox 提交、delivery_key 返回与状态查询）与命令路由（冲突拒绝、管理员校验、确认框架接入）
- [ ] 7.4 插件存储接口与独立迁移执行（`plugins/<name>/migrations/`，SDK 规范第 6 章）
- [ ] 7.5 总线测试：至少一次投递幂等、分发超时、加载失败隔离、命令冲突拒绝、熔断与解除
